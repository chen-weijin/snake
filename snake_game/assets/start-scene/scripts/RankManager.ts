    ...
    report(e, t, n = {}) {
        ...
        if (i) {
            ...
            window.SmSdk.sdkImp.getPlayerInfo(function () {
                ...
                var openid = window.SmSdk.savedOpenid;
                if (!openid || openid.length === 0) {
                    openid = SdkStorage.openid;
                    if (openid && openid.length > 0) {
                        window.sm.log.debug("从SdkStorage获取openid:", openid);
                    }
                }
                if (!openid || openid.length === 0) {
                    openid = "temp_" + Date.now() + "_" + Math.floor(Math.random() * 10000);
                    window.sm.log.warn("openid为空，使用临时openid:", openid);
                }
                ...
                sdkhttp.requestv3(
                    ...
                    JSON.stringify({
                        ...
                        openid: openid,
                        score: t,
                        playername: window.SmSdk.savedPlayerName || "匿名玩家",
